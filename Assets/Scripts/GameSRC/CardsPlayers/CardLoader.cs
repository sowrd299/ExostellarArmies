...
namespace SFB.Game.Content
{
	public class CardLoader : IdIssuer<Card>
	{
		...
		{
			old_listOfCards.Add(name, new UnitCard(cost, name, f, uType, flavor, r, m, hp, a));
		}
		...
		private CardLoader()
		{
			...
			foreach (YamlStream yaml in yamlStreams)
			{
				...
				if (root.Children.ContainsKey(new YamlScalarNode("abilities")))
				{
					...
					AddUnitCard(cardName, cost, faction, unitType, flavorText, r, m, hp, abilities);
				}
				else
				{
					AddUnitCard(cardName, cost, faction, unitType, flavorText, r, m, hp);
				}
			}
